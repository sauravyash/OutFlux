...
let getCertificateFromStoreInfo = exports.getCertificateFromStoreInfo = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (options, vm) {
        ...
        const certList = (0, (_builderUtil || _load_builderUtil()).asArray)(JSON.parse((yield vm.exec("powershell.exe", ["Get-ChildItem -Recurse Cert: -CodeSigningCert | Select-Object -Property Subject,PSParentPath,Thumbprint,IssuerName | ConvertTo-Json -Compress"]))));
        for (const certInfo of certList) {
            if (certificateSubjectName != null) {
                if (!(certInfo.IssuerName.Name.indexOf(certificateSubjectName) !== -1)) {
                    continue;
                }
            ...
            const isLocalMachineStore = parentPath.indexOf("Certificate::LocalMachine") !== -1;
...
function computeSignToolArgs(options, isWin) {
    let vm = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : new (_parallels || _load_parallels()).VmManager();
