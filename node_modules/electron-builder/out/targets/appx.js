        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (userAssetDir == null) {
                ...
                yield (_bluebirdLst2 || _load_bluebirdLst2()).default.map(userAssets, function (it) {
                    return (0, (_fs || _load_fs()).copyFile)(_path.join(userAssetDir, it), _path.join(assetOutDir, it), false);
                });
            }
            ...
            taskManager.addTask((_bluebirdLst2 || _load_bluebirdLst2()).default.map(Object.keys(vendorAssetsForDefaultAssets), function (defaultAsset) {
                if (!isDefaultAssetIncluded(userAssets, defaultAsset)) {
                    return (0, (_fs || _load_fs()).copyFile)(_path.join(vendorPath, "appxAssets", vendorAssetsForDefaultAssets[defaultAsset]), _path.join(assetOutDir, defaultAsset), false);
                }
                return null;
            }));
            const publisher = yield _this.computePublisherName();
            ...
            if (isScaledAssetsProvided(userAssets)) {
                const priConfigPath = vm.toVmFile(_path.join(preAppx, "priconfig.xml"));
                const makePriPath = vm.toVmFile(_path.join(vendorPath, "windows-10", (_builderUtil || _load_builderUtil()).Arch[arch], "makepri.exe"));
                yield vm.exec(makePriPath, ["createconfig", "/cf", priConfigPath, "/dq", "en-US", "/pv", "10.0.0", "/o"], undefined, debug.enabled);
                yield vm.exec(makePriPath, ["new", "/pr", vm.toVmFile(preAppx), "/cf", priConfigPath, "/of", vm.toVmFile(preAppx)], undefined, debug.enabled);
                makeAppXArgs.push("/l");
            }
            ...
            yield vm.exec(vm.toVmFile(_path.join(vendorPath, "windows-10", (_builderUtil || _load_builderUtil()).Arch[arch], "makeappx.exe")), makeAppXArgs, undefined, debug.enabled);
